...
def skeletonize(data: Tensor, kernel: Union[Tensor, None] = None) -> Tensor:
    ...
    Args:
        data: Image with shape :math:`(1, 1, H, W)`. [C = 1]
        kernel: Positions of non-infinite elements of a flat structuring element.
                ...
                For full structural elements use torch.ones_like(structural_element).
    Returns:
        Skeleton of the image with shape :math:`(1, 1, H, W)`. [C = 1]
    ...
    KORNIA_CHECK_SHAPE(data, ["1", "1", "H", "W"])
    ...
    while True:
        ...
        if torch.count_nonzero(data) == 0:
            # TODO: add support to batched version to it
            break
